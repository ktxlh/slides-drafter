    ...
    def split(text):
        ...
        for paragraph in paragraphs:
            ...
            for i in range(len(sents)-1):
                ...
                input_ids = tokenizer.encode_plus(sents[i],sents[i+1], return_tensors='pt')['input_ids']
                logits, attentions = model(input_ids)
                input_ids = input_ids.squeeze(0)
                logits = logits.squeeze(0)
                
                ## Keyphrase is the most attented input
                key_phrase = tokenizer.decode([input_ids[attentions[-1][0,-1,:,0].argmax()]])
                #current sentences = tokenizer.decode(input_ids)
            ...
            # The last sentence
            input_ids = tokenizer.encode_plus(sents[-1], return_tensors='pt')['input_ids']
